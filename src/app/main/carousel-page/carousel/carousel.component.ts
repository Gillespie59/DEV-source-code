...
export class CarouselComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    for (let i = 0; i < this.tabItemContent.length; i++) {
      this.sizeLi =
        this.sizeLi + this.tabItemContent[i].getBoundingClientRect().width;
    }

    // Opacity 0 for All Carousel Item
    for (let i = 0; i < this.tabItemContent.length; i++) {
      this.renderer.setStyle(this.tabItemContent[i], "opacity", 0);
    }

    // Opacity 1 for the first Carousel Item
    this.renderer.setStyle(this.tabItemContent[0], "opacity", 1);
  }
  ...
  next(group: CarouselItemComponent): void {
    ...
    if (index + 1 > lengthGroup - 1) {
      ...
      this.tabItemContent[0].style.transform =
        "translate(+" +
        this.tabItemContent[0].getBoundingClientRect().width +
        "px, 0)";
      this.tabItemContent[lengthGroup - 1].style.transform =
        "translate(-" +
        this.tabItemContent[lengthGroup - 1].getBoundingClientRect().width +
        "px, 0)";
      ...
      this.tabItemContent[index + 1].style.transform =
        "translate(+" +
        this.tabItemContent[index + 1].getBoundingClientRect().width +
        "px, 0)";
      this.tabItemContent[index].style.transform =
        "translate(-" +
        this.tabItemContent[index].getBoundingClientRect().width +
        "px, 0)";
  ...
  previous(group: CarouselItemComponent): void {
    ...
    if (index - 1 < 0) {
      ...
      setTimeout(() => {
        this.tabItemContent[lengthGroup - 1].style.opacity = "1";
        this.tabItemContent[lengthGroup - 1].style.transition =
          "transform 0.3s";
  ...
  goTo(indexToGo: number, group: CarouselItemComponent): void {
    ...
    if (random === 1) {
      this.tabItemContent[indexToGo].style.transform =
        "translate(+" +
        this.tabItemContent[indexToGo].getBoundingClientRect().width +
        "px, 0)";
      for (let i = 0; i < this.tabItemContent.length; i++) {
        this.tabItemContent[i].style.transform =
          "translate(-" +
          this.tabItemContent[i].getBoundingClientRect().width +
          "px, 0)";
      }
    } else if (random === 0) {
      this.tabItemContent[indexToGo].style.transform =
